...
{
	target = targetID;
	huntEndTime = world.time() + random<double>(10000,30000);
	state = STATE_HUNTING;
}
void AIplayer::flyTowardsPoint(shared_ptr<plane> p, Vec3f dest, float strength)
{
	...
	float diffAng = Angle(atan2(destDirection.x, destDirection.z) - p->yaw).radians_plusMinusPI();
	float diffClimb = Angle(atan2(destVector.y, sqrt(destVector.x*destVector.x+destVector.z*destVector.z)) - p->pitch).radians_plusMinusPI();

	p->yaw += clamp(diffAng,1.2,-1.2) * world.time.length()/1000.0 * strength;
	p->pitch += clamp(diffClimb,-0.7,0.7) * world.time.length()/1000.0 * strength;
...
void AIplayer::update()
{
	...
	else if(state == STATE_PATROL)
	{
		...
		for(auto i=planes.begin(); i!=planes.end(); i++)
		{
			if(i->second->team != p->team)
			{
				...
				if(focus1.distance(i->second->position) + focus2.distance(i->second->position) < 5500.0)
				{
	...
	else if(state == STATE_HUNTING)
	{
		...
		//{
			...
			flyTowardsPoint(p, t->position + dynamic_pointer_cast<plane>(t)->velocity / p->velocity * max(distance-2000.0,0.0), strength);
		//}
