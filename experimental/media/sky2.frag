...
// ---------------------------------------- position ------------------------------------------------------------
	...
	gl_FragDepth = min(0.5 + 0.5 * cameraPosition.z / cameraPosition.w, 0.9999999);//min(0.5 + 0.5*( (f+n)/(f-n) - 2.0*f*n/((f-n) * t)),0.99999);
...
// see: http://http.developer.nvidia.com/GPUGems2/gpugems2_chapter16.html
	...
	for(int i=0; i<nSamples; i++)
	{
		...
		float fDepth = exp(fScaleOverScaleDepth * (earthRadius - fHeight));
		float fLightAngle = dot(lightDirection, v3SamplePoint - earthCenter) / fHeight;
	...
	float randomIntensity =	( 0.75 * texture2D(noise, worldPosition.xz*0.000001+0.00001*time*vec2(0.732546,0.680718)).r +
							  0.25 * texture2D(noise, worldPosition.xz*0.000005+0.00004*time*vec2(-0.362358,0.932039)).r);
	float clouds = clamp((exp(randomIntensity)-1.5) * /*s **/ 0.5 * normal.y*normal.y*normal.y * (1.0 - eye.y/ (0.01 * earthRadius)) /** (color.r+color.g+color.b)*/*0.5, 0.0, 0.6);
