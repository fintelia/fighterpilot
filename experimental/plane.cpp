...
//}
plane::plane(Vec3f sPos, Quat4f sRot, objectType Type, int Team):object(Type, Team), lastUpdateTime(world.time()), extraShootTime(0.0),shotsFired(0), cameraRotation(rotation), cameraShake(0.0),firstPerson(nullptr),thirdPerson(nullptr), controlType(CONTROL_TYPE_ADVANCED), locationHistory(1000)
{
	...
	{
		pitch = PI/2 * abs(f.y)/f.y;
		yaw = PI+atan2(u.x,u.z);
	}
	else//only condition that *should* happen
	{
		yaw = atan2(f.x,f.z);
		pitch = atan2(f.y,sqrt(f.x*f.x+f.z*f.z));
	}
	roll = 0;
	velocity=rotation * Vec3f(0,0,400.0);
...
void plane::updateSimulation(double time, double ms)
{
	...
	if(!dead)
	{
		...
		else
		{
			...
			float mass = 29300;
			float speed = velocity.dot(fwd);
			Vec3f Thrust = fwd * (208000.0f + 312000.0f*controls.accelerate - 180000.0f*controls.brake);
			Vec3f Drag = -velocity.normalize() * 0.70743 * velocity.dot(velocity);
			Vec3f Lift = Vec3f(0,0,0);//up * speed * speed *  5.0;
			Vec3f Gravity = Vec3f(0, -9.8* mass * abs(fwd.y)/*max(9.8 * mass, Lift.y)*/, 0); //artificially increase gravity to keep plane level

//			Thrust *= 1.0;	//make game more fast paced
//			Drag *= 1.0;
//			Lift *= 1.0;
			Gravity *= 0.6;

			//Profiler.setOutput("gravity", g);
			Vec3f acceleration = (Thrust + Drag + Lift + Gravity) / mass;
			//Vec3f jerk = fwd * -2.0f * (Drag/(speed*speed)) / (mass*0.1) * speed * acceleration;

			velocity = taylor<float, Vec3f>(ms/1000, velocity, acceleration/*, jerk*/);	
			...
			if(controlType == CONTROL_TYPE_SIMPLE)
			{
				if(fwd.y < 0.99)
				{
					yaw = atan2A(fwd.x, fwd.z);
				}
				else
				{
					yaw = atan2A(-up.x, -up.z);
				}
				float fPitch = asin(fwd.y);
				float yawRate = 0.9*(controls.left - controls.right) * max(PI/3 - abs(fPitch), 0.0f);
				pitch = clamp(fPitch + 0.7*(ms/1000)*(controls.climb - controls.dive), -PI/3, PI/3);
				
				yaw += yawRate * (ms/1000);
				roll = -1.9 * yawRate;

				Profiler.setOutput("pitch", pitch.degrees_plusMinus180());
				...
				//	{
				//		direction -= Lift.magnitude() / (mass*0.2*speed) * sin(roll)/*cos(climb)*/ * (ms/1000);
				//	}
				//	else
				//	{
				//		direction -= Lift.magnitude() / (mass*0.2*speed) * sin(roll)/*cos(climb)*/ * (ms/1000) * 0.2;
				//	}
				//}
				//else
				//{
			...
			else if(controlType == CONTROL_TYPE_AI)
			{
				...
				pitch += (controls.climb - controls.dive) * (ms/1000) * cos(roll);
			}

			if(controlType == CONTROL_TYPE_SIMPLE || controlType == CONTROL_TYPE_AI)
			{
				if(pitch > PI/2 && pitch < PI*3/2)
				{
					yaw += PI;
					roll += PI;
					pitch = PI - pitch;
				...
				rotation = Quat4f(Vec3f(1,0,0),-pitch) * rotation;
				rotation = Quat4f(Vec3f(0,1,0),yaw) * rotation;
			}
			...
			if(ms>0)
			{
				...
				velocity = rotation * lastRotation.conjugate() * velocity;
			}
			////////////////////end move////////////////////////////////////
			...
			if(controls.shoot3 && roll.inRange(-PI/6,PI/6) && pitch.inRange(-PI/4,PI/12))
			{
				if(cameraShake < 0.1)
					cameraShake = 1.0;
	...
	{
		altitude = world.altitude(position);
		velocity.y -= 9.8 * (ms/1000) * (rotation * Vec3f(0,0,1)).y; //gravity
	...
	locationHistory.push_back(position);
}
...
void plane::smoothCamera()
{
	...
	float cameraDistance = objectInfo.planeData(type)->cameraDistance + 8.0 * max(0.0,pow((velocity.magnitude()-300.0)/300.0,3.0));
...
void plane::autoPilotUpdate(float value)
{
	...
	if(time>(wayPoints.back()).time)
	{
		...
		yaw =		atan2A(fwd.x,fwd.z);
		pitch =		asin(fwd.y/fwd.magnitude());
		roll =		0;//too hard and rather useless to find
...
void plane::returnToBattle()//needs to be adjusted for initial speed
{
	...
	if(endpoint.distanceSquared(world.bounds().center) > world.bounds().radius*world.bounds().radius - 10.0)
	{
	...
	velocity = wayPoints.back().rotation * rotation.conjugate() * velocity;
...
void plane::shootMissile()
{
	...
	world.newObject(new missile(t, team, lastPosition+right*o.x+up*o.y+fwd*o.z, rotation, velocity.dot(fwd), id, pId)); //lastPosition is used since the missile will update this frame and move to under the plane
...
void plane::dropBomb()
{
	...
	world.newObject(new bomb(t, team, lastPosition+right*o.x+up*o.y+fwd*o.z, rotation, velocity.dot(fwd), id)); //lastPosition is used since the bomb will update this frame and move to under the plane
