...
bool levelEditor::init()
{
	...
	view->setRenderFunc(std::bind(&levelEditor::render3D, this, 0));
...
void levelEditor::updateFrame()
{
	if(buttons["load"]->checkChanged())
	{
		...
		p->callback = [this, p]{
			...
			if(!levelFile.loadZIP(p->getFile()))
				debugBreak();
	...
	{
		saveFile* p = new saveFile;
		p->callback = [this, p]{
			if(!p->validFile()) return;
			...
			if(!levelFile.saveZIP(p->getFile(),pow(10.0f,sliders["height scale"]->getValue()), sliders["sea level"]->getValue()))
