...
splitScreen::splitScreen(shared_ptr<const LevelFile> lvl): dogFight(lvl), countdown(0.0), restart(false), levelup(false), victory(false)
{
	...
	views[0]->setRenderFunc(std::bind(&splitScreen::render3D, this, 0));
	views[0]->setTransparentRenderFunc(std::bind(&splitScreen::renderTransparency, this, 0));

	views[1] = graphics->genView();
	views[1]->viewport(0, 0.5, sAspect, 0.5);
	views[1]->perspective(40.0, (double)sw / ((double)sh/2),1.0, 500000.0);
	views[1]->setRenderFunc(std::bind(&splitScreen::render3D, this, 1));
	views[1]->setTransparentRenderFunc(std::bind(&splitScreen::renderTransparency, this, 1));

	graphics->setLightPosition(Vec3f(0.0, 16000.0, 10000.0));
}
bool splitScreen::init()
{
	vector<planeType> planes(2, objectInfo.getDefaultPlane());
