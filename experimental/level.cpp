...
void LevelFile::initializeWorld(vector<planeType> playerPlanes) const
{
	...
	for(auto i = objects.begin(); i != objects.end(); i++)
	{
		if(i->type == PLAYER_PLANE && players.numPlayers() < playerPlanes.size())
		{
			auto id = world.newObject(new plane(i->startloc, i->startRot, playerPlanes[players.numPlayers()], i->team));
	...
	for(auto i = objects.begin(); i != objects.end(); i++)
	{
		if(i->type & PLANE && i->type != PLAYER_PLANE)
		{
			...
			{
				auto id = world.newObject(new plane(i->startloc, i->startRot, playerPlanes[players.numPlayers()], i->team)); //keep creating objects as though they were marked PLAYER_PLANE
				players.addHumanPlayer(id);
			}
			else
			{
	...
	if(players.numPlayers() > 0 && players.numPlayers() < playerPlanes.size()) //add additional human players if necessary
	{
		for(int i=players.numPlayers(); i<playerPlanes.size(); i++)
		{
