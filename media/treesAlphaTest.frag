...
void main()
{
	...
	color.a *= clamp((6500.0-distance(eyePos,position))*0.002,0.0,1.0);
	if(color.a < 0.5)
	{
		discard;
	}
...
//		gl_FragColor = vec4(mix(color.rgb, textureCube(sky, vec3(-eyeDirection.x,0,-eyeDirection.z)).rgb, clamp(0.000000001*dot(eyeDirection,eyeDirection),0.0,1.0)),1.0);
		...
		float fogAmount = 1-min(5000.0/length(eyeDirection), 1);

		gl_FragColor.rgb = mix(treeColor * color.rgb, fogColor, fogAmount);
		gl_FragColor.a =
		    clamp((7500.0 - distance(eyePos, position)) * 0.0005, 0.0, 1.0);
