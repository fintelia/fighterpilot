...
public:
	...
	double distanceToGround(Vec3f point)
	{
		...
		Vec3f tmp1(sx+1,getHeight(sx+1,sz),sz);
		Vec3f tmp2(sx,getHeight(sx,sz+1),sz+1);
		Vec3f v1((tmp1-tmp2));
		Vec3f normal;
		if((x-sx)*(x-sx)+(z-sz)*(z-sz)<=(x-sx-1)*(x-sx-1)+(z-sz-1)*(z-sz-1))
		{
			Vec3f tmp3(sx,getHeight(sx,sz),sz);
			normal=v1.cross(tmp1-tmp3).normalize();
			vert[0]=sx;
		...
		{
			Vec3f tmp3(sx+1,getHeight(sx+1,sz+1),sz+1);
			normal=v1.cross(tmp1-tmp3).normalize();
			vert[0]=sx+1;
